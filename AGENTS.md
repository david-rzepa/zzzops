...
- Goal files are work truth; keep stable paths/backlinks. Triage `new`; mark `done` only from observed criteria and recheck parents. By default dependencies must be complete before writable implementation; reviewed PROJECT policy may override actionability while preserving ancestry and merge order.
...
- PROJECT policy is the operational source of truth. Parallel permission is a ceiling: workers are read-only unless `worktrees`; coordinator alone edits ZzzOps state/integrates commits. Refill only when reviewed policy explicitly opts in.
...
Run the root `install.ps1` or `install.sh` only from this base repository. They install discoverable skills/mechanics into targets but never themselves, project state, or target `AGENTS.md`/`CLAUDE.md`.
