...
The canonical policy preserves extension settings inside policy sections. GitHub Issues is authoritative only after identity, authentication, Issues, and permission probes pass; a failed probe is an explicit blocker with no fallback authority. After initialization, `checkpoint` combines those checks with the complete canonical portfolio in one CLI call. This first release intentionally contains no prior-schema migration machinery.
