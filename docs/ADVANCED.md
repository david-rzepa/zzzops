...
The optional `agent_observability` category identifies a specific diagnostic question that current repository signals cannot answer without costly inference or user help. It proposes the smallest safe, repository-native signal—such as a read-only debug/MCP query, structured diagnostic logs, or a bounded profile—only when existing diagnostics are inadequate. Existing projects remain opted out until their category list is explicitly reviewed.
...
Preview never records review coverage. It reports the selected scope and either evidenced findings or an explicit no-findings result without turning an empty inbox into a cleanliness claim. An explicitly completed manual review—or one recent review explicitly invoked by execution after true exhaustion—records only its exact frozen manifest, so interruption, changed goal revisions, or changed PR heads leave the review due with an actionable recovery path rather than silently advancing it.
