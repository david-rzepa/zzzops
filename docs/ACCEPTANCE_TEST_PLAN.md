...
| A-012 | Have two independent runs reserve the same goal revision simultaneously, then renew/release the winner and repeat after expiry. | Exactly one run wins each contention; the loser refreshes and chooses other work, only the winner can renew/release, and expired work becomes available without deleting a newer reservation. |
