...
Execution performs no scheduled entropy scan or changed-line analysis. A qualifying observed checkpoint, integration, or completion causes one compact exact-event write; concrete out-of-scope decay already exposed by ordinary work causes one small atomic inbox write. Only at refreshed true exhaustion does execution read compact review status, and it cold-loads the recent entropy workflow only when an exact batch is due. Every explicit work-suggestion run reads the compact inbox once and spends audit tokens only validating policy-eligible observations; excluded categories remain unread in the returned evidence set.
