...
class PolicyCleanupTests(unittest.TestCase):
    ...
    def test_quality_and_autonomy_defaults_expose_only_active_choices(self):
        self.assertEqual(set(), set(self.section("code_quality")["configuration"]))
        self.assertEqual({
            "max_workers", "execution_reports", "resource_reservations", "refill",
        }, set(self.section("autonomy_approval_parallelism")["configuration"]))
    ...
    def test_customized_worker_resource_constraints_remain_valid(self):
        ...
        autonomy["configuration"]["max_workers"] = 2
        autonomy["instructions"] += " Use read-only workers above 50 MiB of tracked files."
        autonomy["configuration"]["resource_reservations"]["mode"] = "strict"
        autonomy["instructions"] += " Retain clean worktrees for reuse."
    ...
    def test_required_ci_modes_are_explicit(self):
        for mode in ("inspect_exact_pr_head", "disabled", "existing_only"):
            ...
            verification["configuration"]["required_ci"] = mode
        ...
        verification["configuration"]["required_ci"] = "assume_green"
    ...
    def test_removed_knobs_make_an_old_policy_require_review(self):
        ...
        quality["configuration"]["completion_self_review"] = "required_before_review_or_done"
        autonomy = next(item for item in stale["sections"] if item["id"] == "autonomy_approval_parallelism")
        autonomy["configuration"]["claim_ttl_hours"] = 4
        errors = policy.validate_policy(stale, True)
        self.assertTrue(any("code_quality.configuration" in error for error in errors))
        self.assertTrue(any("autonomy_approval_parallelism.configuration" in error for error in errors))
    ...
    def test_adopted_old_default_is_reported_as_update_available(self):
        ...
        legacy_content["configuration"]["completion_self_review"] = "required_before_review_or_done"
        quality.pop("default_id")
        quality["instructions"] = legacy_content["instructions"]
        quality["configuration"] = legacy_content["configuration"]
        quality["default_provenance"] = {
            "status": "adopted", "default_id": current["id"], "schema_version": policy.POLICY_DEFAULT_SCHEMA_VERSION,
