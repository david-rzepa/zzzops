...
2. Consolidate duplicates, search evidence, then apply PROJECT `blocker_order` (fallback: safety/access/human; cross-goal choices; specifications; technical unknowns).
   For design-only blockers, read `[policy:automated_design]`. If enabled, compare reversible in-scope options using objectives, KPI evidence, constraints, and precedence; record alternatives, rationale, assumptions, and a falsifiable validation signal. Privacy/security requires an option that unambiguously reduces exposure, privilege, collection, or retention without material behavior/compatibility change. Disabled policy, weak/conflicting evidence, or consequential ambiguity requires a durable design blocker. Never authorize product scope, incompatible public contracts, destructive migrations, external spending, deployment, external writes, bypassed review, or weaker safety/authority.
