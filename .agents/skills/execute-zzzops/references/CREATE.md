...
2. Promptly create a provisional canonical goal using `BACKENDS.md`: a managed issue or a local file from `.agents/templates/project-goals/GOAL.md`; use PROJECT capture defaults and override them only with evidence.
...
4. If consequential ambiguity remains, follow `.zzzops/rules/BLOCKERS.md`: record it, set `needs_human`, queue it, and interview per PROJECT policy with recommendations and continuation boundaries. Proceed on reversible cosmetic assumptions; never speculate across foundational choices.
5. Write independently verifiable criteria and explicit scope. Avoid vague completion language.
6. Map parent/dependencies, then create a child only for a separately verifiable, independently prioritized/blocked/claimed outcome or distinct risk/workstream; use parent checklists otherwise. Obey PROJECT depth/required-child policy; update both directions and reject cycles.
7. Set priority/value/difficulty/confidence/owner/dates from `.zzzops/PROJECT.md`, `.zzzops/rules/GOAL_SYSTEM.md`, and analogous local-ledger evidence. State which acceptance criterion/KPI/beneficiary the goal advances; if the charter cannot support a value judgment, keep confidence low and record/ask about the gap. Preserve user budgets; otherwise bound a checkpoint, not invented token cost.
