...
2. Run `report list` and inspect every valid archived report. Each automatic report contains only a constrained machinery cause code and numeric impact; malformed or unknown content is a safety failure, so stop without submitting or deleting anything.
