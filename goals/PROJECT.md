# Project success charter

<!-- zzzops-project-state
{
  "backend": null,
  "initialized": false,
  "repository": null,
  "revision": 0,
  "schema_version": 1
}
zzzops-project-state -->

**Status:** incomplete
**Last reviewed:** not yet

## Overall goal
- Outcome: unknown
- Primary beneficiaries: unknown
- Why it matters: unknown
- Time horizon: unknown

## Success metrics
| KPI | Why it matters | Baseline | Target / threshold | Evidence source | Review cadence |
| --- | --- | --- | --- | --- | --- |
| Unknown | Unknown | Unknown | Unknown | Unknown | Unknown |

## Project acceptance criteria
- [ ] Define observable conditions that collectively mean the project has succeeded.

## Value rubric
- `critical`: required for project acceptance, safety, or a binding deadline.
- `high`: materially moves a priority KPI or unlocks critical/high-value work.
- `medium`: useful measurable contribution with limited leverage.
- `low`: weak, speculative, cosmetic, or currently unmeasured contribution.

When KPIs conflict, prefer: unknown; ask the user.

## Constraints and non-goals
- Constraints: unknown
- Non-goals: unknown
- Unacceptable tradeoffs: unknown

## Assumptions and open questions
- Complete this charter with the user before assigning confident goal value.

## History
| Date | Actor/run | Change | Reason/evidence |
| --- | --- | --- | --- |
| Not yet | installer | Initialized incomplete charter | Requires project-specific user input |
