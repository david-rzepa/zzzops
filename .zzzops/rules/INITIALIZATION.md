...
1. Resolve Python 3 once before first CLI use: prefer harness runtime discovery; otherwise probe `python3`, `python`, and Windows `py -3` together. Reuse that exact `<python>` command, never hard-code a harness path, and block once with actionable detail if absent.
2. For a reviewed project run `<python> .agents/zzzops/zzzops.py --repo . checkpoint` once. Under Codex, use a scoped approved authenticated context for the first attempt at this checkpoint and `gh` paths; keep local-only commands in the normal sandbox. Never reauthenticate or persistently relax the sandbox. Elsewhere retry the same bounded command once after unexpected auth failure. The checkpoint requires installed machinery tracked and unchanged from `HEAD` (excluding policy/state/root instructions); dirty, untracked, or deleted machinery requires a policy-consistent commit. Continue only on `ready:true` using its complete/valid portfolio; do not run another portfolio command.
