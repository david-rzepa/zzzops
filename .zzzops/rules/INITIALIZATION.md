...
2. For ordinary reviewed projects run `<python> .agents/zzzops/zzzops.py --repo . checkpoint` once. Under Codex, use a scoped approved authenticated context for the first attempt at this checkpoint and `gh` paths; keep local-only commands in the normal sandbox. Never reauthenticate or persistently relax the sandbox. Elsewhere retry the identical bounded command once after unexpected auth failure. After initialization the checkpoint first requires installer-managed skills/rules, CLI/templates, and machinery ignore files to be tracked and unchanged from `HEAD`; policy/state and root instructions are excluded. Dirty, untracked, or deleted machinery stops for a policy-consistent commit. Continue only on `ready:true`; use the complete/valid embedded portfolio and never run a second portfolio command at that checkpoint.
