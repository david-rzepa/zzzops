...
All installed agent workflows do this before ordinary work. The external native `install.ps1` and `install.sh` scripts do not:
...
6. Stop until the user explicitly confirms review. Then run `init confirm --project-digest DIGEST --reviewer NAME --all` (or repeated `--section ID`). Only that command checks sections. Any file change makes the digest stale; unchecked required sections remain `decision` blockers. Confirmation compacts `PROJECT.md`; hashed `PROJECT_AUDIT.md` keeps evidence/history out of routine context.
7. Run `checkpoint` before ordinary work. Initialization makes no Git/GitHub writes. After success, continue with the reviewed PROJECT policy as the single operational policy surface.
