...
When the user is available, blocker review precedes normal prioritization: search project/related goals just enough to phrase useful questions, then interview immediately in one compact categorized batch with a recommended default/consequence and what continues. Do not spend the interactive window on speculative implementation first. Persist questions; do not repeat before their recheck trigger. For `local_files`, put every open human blocker in derived `goals/INDEX.md` with goal, category, request, impact, and date; GitHub uses managed issues.
