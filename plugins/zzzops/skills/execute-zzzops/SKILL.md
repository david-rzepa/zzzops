...
First invoke `zzzops workflow --intent execute --source-skill '$execute-zzzops'` and obey its action-only `next_steps`. Do not invoke a private ZzzOps command from this skill or decide whether a phase should be delegated. Only a `delegate` next step authorizes delegation, and its named `model` and `effort` are mandatory launch parameters; a `continue_root` step requires root execution, while `resolve_blocker` must be cleared and retried. The workflow response determines when `../../rules/INITIALIZATION.md` and `../../rules/BACKENDS.md` apply; then load only the named rules, goal context, and references.
