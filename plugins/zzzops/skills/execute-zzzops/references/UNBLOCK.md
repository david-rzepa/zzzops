...
2. Consolidate duplicates and search evidence. Resolve safety, access, and human-authority blockers before cross-goal decisions, specification gaps, and technical unknowns.
...
5. Migration blockers link the preparation format and current bindings: investigate, prepare the local evidence file, then recheck. Persist only unresolved questions through the returned block contract. Preserve shipped contracts; only evidenced unreleased project-owned state may be eligible for bounded replacement within existing authority. Never infer that state can be wiped.
6. At exhaustion, give ordered issue links and what answers resume. Do not poll, watch, notify repeatedly, or await a response.
7. Answers resolve, not delete, blockers; record answer, resolver/date, changed scope/criteria/next action, and successors; rebuild and resume.
