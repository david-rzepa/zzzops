...
5. For migration blockers, use the release-aware `legacy_migration` policy evidence before proposing a workaround: missing settings require policy re-review, ambiguous evidence remains blocked, and only an explicit `never_released` decision authorizes bounded replacement of affected project-owned state. Never infer that state can be wiped from absent tags, version strings, or an empty release list.
