...
1. Use the charter and current public workflow checkpoint; require its prerequisite and portfolio repairs before selecting. Use the returned relationships, claims, and reviews without rereading unrelated goals. If the human queue exists, use `UNBLOCK.md` to persist/order gates, then continue independent work. Treat the checkpoint as the current queue read until mutation, failure, freshness, or drift requires one public reread.
...
Update only for new result, decision, risk, changed assumption, required action, or long-operation heartbeat; never recap unchanged state. Before substantive work on a newly selected goal, state outcome/scope.

## Execute

1. Re-read only the selected goal and critical parent/dependencies through the returned public read or repair step; match revision/digest and follow the returned phase lease contract. On contention refresh once and switch; only the winner starts. Repair a legacy schema/body only through the public workflow step returned for that goal—never sweep others.
...
- Out-of-scope decay: preserve the bounded evidence for the mandatory implementation review as described in [incidental entropy observations](ENTROPY_OBSERVATIONS.md); do not audit.
...
When no goal has `work_state: triage|prepare|write`, rebuild blockers through `UNBLOCK.md`, then follow [the PR review queue](REVIEW_QUEUE.md) for the current public checkpoint, optional policy-authorized suggestion, and ordered final handoff. Later resolve supplied input and resume; never poll for humans.
