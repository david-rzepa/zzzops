...
Run this prompt only when the public workflow returns this exact step. Check requirements, architecture, risks, and finite leaf output scope against the goal and returned review requirements. Identify unresolved human decisions in review evidence; only root may ask the human in a separate step.
...
Keep review independent from the executor. If changes are needed, submit the findings in the review evidence and let the workflow route correction and fresh review.
