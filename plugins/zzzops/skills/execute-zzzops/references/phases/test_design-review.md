...
Run this prompt only when the public workflow returns this exact step. Independently verify the criterion-to-test trace, baseline failure, exclusions, and that the tests represent the requested behaviour.
...
Keep review independent from the executor. If changes are needed, submit the findings in the review evidence and let the workflow route correction and fresh review. Record any needed human decision for a separate root step; a worker never asks the human.
