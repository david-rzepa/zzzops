...
Run this prompt only when a `zzzops workflow` checkpoint returns this exact step. Split a non-atomic parent into independently deliverable child goals; mark the phase not required only under the supplied atomic-goal policy rule.
