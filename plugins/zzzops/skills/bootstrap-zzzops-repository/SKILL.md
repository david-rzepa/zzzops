...
Codex plugins do not put a `zzzops` binary on `PATH`. From this loaded skill's absolute `<skill><path>`, derive `ZZZOPS_CLI` by replacing `skills/bootstrap-zzzops-repository/SKILL.md` with `zzzops/zzzops.py`. Invoke it as `python3 "$ZZZOPS_CLI"`; do not search for, install, or invoke a global `zzzops` command.

First invoke `python3 "$ZZZOPS_CLI" workflow --intent inspect --source-skill '$bootstrap-zzzops-repository'` and obey its `next_steps`. Do not invoke a private ZzzOps command named elsewhere in this skill; those references describe the workflow handler behind this public command.
...
3. Read [goal-DAG planning](../../zzzops/references/bootstrap/PLAN.md). Create or reconcile exactly one canonical top-level product-outcome goal before harness goals, then place justified harness outcomes beneath it with only their real dependencies. After tooling/readback and harness-goal creation, persist the post-harness review checkpoint; do not create, claim, branch, or execute product implementation goals until that checkpoint is explicitly approved.
...
5. Invoke `$execute-zzzops` for the authorized canonical DAG. Resume from an approved checkpoint explicitly and idempotently. Continue from harness outcomes into product milestones through distinct stacked PRs until no [[safe useful work]](../../concepts/safe-useful-work.md) remains. Unresolved product authority or high commitment blocks only the affected chain; required checks, PR review, merge, external-write, deployment, and release gates remain.
