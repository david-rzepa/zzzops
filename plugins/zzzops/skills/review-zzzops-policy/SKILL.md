...
Always show `capabilities.release_status` and `capabilities.legacy_migration_review` alongside the policy table. `legacy_migration` is mandatory for a new or explicitly re-reviewed policy; if an existing policy lacks it, treat execution as uninitialized and require explicit policy re-review before migration work. A public GitHub Release is release evidence; an absent release is ambiguous unless the owner explicitly records `never_released`. Route ambiguous evidence to an interactive decision or a durable unattended blocker. A recorded `never_released` policy becomes stale at the first detected release and must be re-reviewed before any state replacement or reset.
