...
3. During an active loop, an additive capture or a user answer that resolves a surfaced blocker updates the queue at the next safe checkpoint; never nest/duplicate execute or capture. After resolution, rebuild the available-work set and resume once when same-task intent remains; explicit stop/replacement wins.
