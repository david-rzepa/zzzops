...
For preview, keep preflight read-only: run status/checkpoint reads, but stop and report if initialization would require installation validation, policy reconciliation, or another state-changing handoff. Preview never calls `report record` or persists execution reports.

Choose the narrowest requested mode and action:

- `recent` is the default. It reviews only the current uncovered event batch. Read [the recent review contract](../../../skills/review-zzzops-entropy/references/RECENT.md).
- `full` requires an explicit full or repository-wide request. It ignores prior coverage for scope selection. Read [the full review contract](../../../skills/review-zzzops-entropy/references/FULL.md).
...
- `apply` requires an explicit user request or the caller's reviewed exhausted-queue refill authority. It creates approved findings with `$add-zzzops-goal` semantics after applying the existing suggestion ranking, category, cap, and refill-authority contracts. Automatic invocation alone grants none. In automatic recent mode, record the exact completion receipt before any apply handoff. Do not invoke `$suggest-zzzops-work` as a second broad audit. This skill never edits source, policy, tests, CI, or documentation directly.
...
6. Complete only after every selected scope item was actually inspected and no interruption remains. Rehydrate the current goal revisions and PR heads, derive the current exact event IDs, then pass a bounded temporary JSON object containing `schema_version:1`, the exact `batch_id`, `outcome:clean|findings`, and those derived IDs as `current_events` to `entropy review complete --input FILE`. Delete only that temporary request after the command. Drift, malformed state, or failure leaves coverage due; never retry with altered identifiers or claim completion.
7. Preview stops after the report. Explicit manual apply remains independent of completion. Automatic recent returns its fixed reviewed findings only after step 6 records the exact receipt; the caller may then apply them through the existing suggestion/capture contracts and approval rules. It does not launch another discovery pass or invent a second goal-creation path. If post-receipt capture fails, persist the fixed finding set and an actionable continuation so completed coverage cannot lose it.
