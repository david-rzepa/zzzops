...
Show release facts separately from standing policy approval. Classify the affected contract, not the whole project: preserve shipped behavior; require captured scoped agent investigation or owner evidence for unreleased replacement. Do not demand a human statement when inspected immutable facts resolve the contract; missing releases alone prove no such boundary. Missing or ambiguous evidence blocks affected work, not unrelated execution. Keep genuine configuration/content approval gates. The unshipped `legacy_migration.release_status` setting is retired; propose its removal through explicit policy review, never silently adopt it.
