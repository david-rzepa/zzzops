...
The installer requires Python 3.10 or newer, previews one exact disposable machinery set and its incoming version, and defaults every confirmation to no. Upgrade previews show the installed and incoming versions. A confirmed fresh install, reinstall, repair, or upgrade wipes only the scoped ZzzOps-owned roots, reconstructs them from the source clone, validates every SHA-256 digest, updates exact ignore entries, writes `.zzzops/ZZZOPS_LOCK.json` last, and confirms the installed version. Local edits to installed machinery are disposable; rerun the regular installer to repair them. Use `-DryRun` on Windows or `--dry-run` on macOS/Linux for a non-interactive preview.

To restore the exact version already pinned by a project, run `install.ps1 TARGET -Restore` on Windows or `install.sh TARGET --restore` on macOS/Linux. Restore verifies the target lock first, resolves its revision locally or fetches it from the ZzzOps clone's `origin`, checks every locked source digest before target changes, and reconstructs through a temporary worktree without switching the clone's current checkout. It preserves the target lock exactly and fails safely if the revision is unreachable or its source no longer matches the lock.
